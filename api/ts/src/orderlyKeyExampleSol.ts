...
async function addOrderlyKey(): Promise<void> {
  ...
  var [message,messageToSign] = addOrderlyKeyMessage(orderlyKey, BROKER_ID , "read","undefined", CHAIN_ID,365)
  console.log(message)
...
function addOrderlyKeyMessage(orderlyKey: string, brokerId:string, scope:string, tag:string, chainId: number, expiration:number) {
  const message = {
    brokerId: brokerId,
    chainType: "SOL",
